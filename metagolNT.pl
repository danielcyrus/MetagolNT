...
randomlySelect(_, _, 0, Out, OutExamples):-
    OutExamples = Out.

randomlySelect(Examples, Length, Count, Out, OutExamples):-
    random_between(0,Length,Rand),
    nth0(Rand, Examples, Elem),
    not(contains_term(Elem,Out)) ->
    (Count1 is Count - 1,  
    append([Elem], Out, Out1),  
    randomlySelect(Examples,Length,Count1, Out1,OutExamples));
    randomlySelect(Examples,Length,Count, Out,OutExamples).

testSelect([],NE, Out,Tests):-
    Tests = Out.
testSelect([H|T],NE, Out,Tests):-
    not(contains_term(H,NE)) ->
    (append(Out,[H],Out1),
    testSelect(T,NE,Out1,Tests));
    testSelect(T,NE,Out,Tests).

trainTestSamples(POS,NEG,NoisLevel,TrainPos,TestPos,TrainNeg,TestNeg):-
    length(POS, Npos),
    length(NEG, Nneg),
    NP is (Npos * NoisLevel)  / 100,
    NN is (Nneg * NoisLevel)  / 100,
    NoisP is round(NP),
    NoisN is round(NN),
    Npos1 is Npos - NoisP,
    Nneg1 is Nneg - NoisN,
    randomlySelect(POS,Npos,Npos1,[],TrainPos),
    randomlySelect(NEG,Nneg,Nneg1,[],TrainNeg),
    testSelect(POS,TrainPos,[],TestPos),
...
evaluate(TestPos,TestNeg,Accuracy,TP,TN,PP,PN):-
      posGrade(TestPos,0,PosGrade),
      negGrade(TestNeg,0,NegGrade),
      length(TestPos, LenPos),
      length(TestNeg, LenNeg),
      TP = PosGrade, 
      TN = NegGrade,
      PP is LenPos - PosGrade,
      PN is LenNeg - NegGrade,
      Accuracy is (PosGrade+NegGrade) / (LenPos+LenNeg).

learn_NT(_,_,_,0,_,_,H,Hyp):-
    Hyp = H.
